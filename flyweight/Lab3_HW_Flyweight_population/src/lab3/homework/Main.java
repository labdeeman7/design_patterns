...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            ArrayList<Integer> inhabitantAges = new ArrayList<>();
            ArrayList<Integer> inhabitantIndices = new ArrayList<>();
            ArrayList<Boolean> inhabitantDeathStatuses = new ArrayList<>();
            ArrayList<Integer> inhabitantDaysInHospital = new ArrayList<>();
            ...
            for (int j = 0; j < 5; j++) {
                ...
                Person human;
                human = PersonFactory.getPerson(probsHospital, probsHospitalDeath,
                         probsSuddenDeath, sex);
                people.add(human);

//                if (j ==2 ) throw new ArithmeticException("2");
//                System.out.println("sex = " + human. .toString());
                //people.add(new Person(probsHospital,probsHospitalDeath,probsSuddenDeath,age,sex,i*5+j));
                inhabitantAges.add(age);
                inhabitantIndices.add(i*5+j);
                inhabitantDeathStatuses.add(false);
                inhabitantDaysInHospital.add(-1);

            }
            houses.add(new Household(people,i,inhabitantAges,inhabitantIndices, inhabitantDeathStatuses, inhabitantDaysInHospital));
        }
